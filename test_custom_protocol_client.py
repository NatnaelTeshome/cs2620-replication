...
def test_decode_data_bytes_list_accounts():
    ...
    data_bytes = struct.pack("!I", total) + struct.pack("!H", account_count) + b"".join(parts)
...
def test_decode_push_event_new_message():
    ...
    payload = struct.pack(fmt, EVENT_NEW_MESSAGE, msg_id, len(sender), sender, len(content), content, ts)
...
def test__decode_response_payload():
    ...
    payload = struct.pack("!B H", success, len(message)) + message + struct.pack("!H", len(data_bytes)) + data_bytes
...
def test_cp_list_accounts(monkeypatch, cp_client):
    def fake_send_request(payload):
        return {"success": True, "message": "Listed", "data": {"accounts": ["alice", "bob"]}}
    monkeypatch.setattr(cp_client, "_send_request", fake_send_request)
...
def test_cp_read_messages(monkeypatch, cp_client):
    def fake_send_request(payload):
        return {"success": True, "message": "Read", "data": {"read_messages": [{"id": 1}, {"id": 2}]}}
    monkeypatch.setattr(cp_client, "_send_request", fake_send_request)
